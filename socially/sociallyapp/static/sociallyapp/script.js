$(document).ready(function() {
    ...
    const $gptSuggestionButton = $('#gpt-suggestion-button');
    const $gptSuggestionBox = $('#gpt-suggestion-box');

    // Function to check if the input has three or more words
    function checkInput() {
        const text = $postContent.val().trim();
        const wordCount = text.split(/\s+/).length;
        if (wordCount >= 3) {
            $gptSuggestionButton.prop('disabled', false);
        } else {
            $gptSuggestionButton.prop('disabled', true);
        }
    }

    // Check input length on keyup
    $postContent.on('keyup', checkInput);

    // Handle Get GPT's Suggestion button click
    $gptSuggestionButton.on('click', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                $gptSuggestionBox.html(`<div class="alert alert-info">${response.response}</div>`).show();
            },
            error: function(xhr) {
                $gptSuggestionBox.html('<div class="alert alert-danger">Failed to get GPT suggestion. Please try again.</div>').show();
            }
        });
    });
    ...
    $('#post-button').click(function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $gptSuggestionBox.empty().hide(); // Ensure the suggestion box is hidden after posting
